...
const typeLabel = {
  token:     { color: c.redBold,  label: 'Exposed Token' },
...
function printConsoleReport({ findings, stats }, options = {}) {
  ...
  const typeOrder = ['token', 'sdk', 'framework', 'endpoint', 'model'];
  ...
  if (uniqueTokens.length > 0) {
    console.log(c.red(`    Tokens:      ${uniqueTokens.join(', ')}`));
  }
  console.log('');

  // ── Verdict ──
  if (stats.criticalFindings > 0) {
    console.log(c.critBg('\n  🚨 CRITICAL: Exposed API tokens detected! Rotate these keys immediately. \n'));
  } else if (stats.highFindings > 0) {
    console.log(c.highBg('\n  ⚠️  WARNING: Potential hardcoded credentials found. Review and remediate. \n'));
  } else {
    console.log(c.okBg('\n  ✅  No exposed tokens. AI SDKs and frameworks detected for awareness. \n'));
  }
}
