...
const { LLM_SDKS, AI_FRAMEWORKS, TOKEN_PATTERNS, API_ENDPOINTS, MODEL_REFERENCES, GENERIC_SECRETS } = require('./patterns');
...
class Scanner {
  constructor(options = {}) {
    ...
    this.aiOnly = options.aiOnly === true;   // opt-in: skip generic secret patterns
    this.findings = [];
    this.stats = {
      filesScanned: 0,
      filesSkipped: 0,
    ...
    for (const patternDef of patterns) {
      ...
      while ((match = patternDef.pattern.exec(content)) !== null) {
        ...
        if (trimmedLine.startsWith('//') || trimmedLine.startsWith('#') || trimmedLine.startsWith('*') || trimmedLine.startsWith('/*')) {
          // Still report tokens and secrets in comments — they're still exposed!
          if (patternDef.type !== 'token' && patternDef.type !== 'secret') continue;
        }

        // For tokens and secrets, mask the actual value
        let matchedText = match[0];
        if (patternDef.type === 'token' || patternDef.type === 'secret') {
  ...
  scan() {
    ...
    // Generic secrets included by default, skip with --ai-only
    if (!this.aiOnly) {
      allPatterns.push(...GENERIC_SECRETS);
    }
    ...
    this.findings = this.findings.filter(f => {
      if (f.context === 'docs') {
        // Only keep exposed tokens and secrets — those are dangerous even in docs
        return f.type === 'token' || f.type === 'secret';
      }
      return true;
    });
